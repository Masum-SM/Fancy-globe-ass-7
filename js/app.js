...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product gy-3">

      <div>
         <img class="product-image" src=${image}></img>
      </div>
      <h5 class="fw-bold mt-3">${product.title.slice(0,36)}</h5>
      ...
      <small class="rating"><i class="fas fa-user fs-6"></i> <sapn class="text-dark">${product.rating.count}</sapn></small>
    
     <div class="c-footer text-center">
        <h3 class="mt-5 text-center">Price:<span class="product-price"> $${product.price}</span></h3>
        <div><button onclick="addToCart(${product.price})"  id="addToCart-btn" class="buy-now  card-btn">add to cart</button>
        <button id="details-btn" class="card-btn">Details</button></div>
...
const updatePrice = (id, value) => {
  ...
  document.getElementById(id).innerText = fixedTotal;
};
